...
public class CMUtil {
	...
	public int calculateTotalCpu() throws Exception {
		//String yarnRoles = getMethod("http://192.168.8.206:7180/api/v9/clusters/cluster/services/yarn/roles");
		String yarnRoles = getMethod(InitConfig.CM_ADDR + "/api/v9/clusters/" + getClusterName() + "/services/" + getYarnServiceName() + "/roles");
		Map<String, Object> NodeManagerNumMap = parseNodeManagerNum(yarnRoles); 
		System.out.println("NodeManagerNumMap>>>>>>>>>>>>>>>>>>");
		System.out.println(NodeManagerNumMap);
		//String roleConfigGroups = getMethod("http://192.168.8.206:7180/api/v9/clusters/cluster/services/yarn/roleConfigGroups");
		String roleConfigGroups = getMethod(InitConfig.CM_ADDR + "/api/v9/clusters/" 
		           + getClusterName() + "/services/" + getYarnServiceName() + "/roleConfigGroups");
		System.out.println("roleConfigGroups>>>>>>>>>");
		System.out.println(roleConfigGroups);
		int totalCpu = 0;
		JSONArray roleConfigGroups_ja = (new JSONObject(roleConfigGroups)).getJSONArray("items");
		for (int i = 0; i < roleConfigGroups_ja.length(); i++) {
			JSONObject roleConfigGroup_jo = roleConfigGroups_ja.getJSONObject(i);
			if (!"NODEMANAGER".equals(roleConfigGroup_jo.getString("roleType"))) {
				continue;
			}
			JSONArray config_ja = roleConfigGroup_jo.getJSONObject("config").getJSONArray("items");
			int NodeManagerGroupNum = (int) NodeManagerNumMap.get(roleConfigGroup_jo.getString("name"));
			int nodeManagerGroupCpu = 0;
			for (int j = 0; j < config_ja.length(); j++) {
				JSONObject item_jo =config_ja.getJSONObject(j);
				if ("yarn_nodemanager_resource_cpu_vcores".equals(item_jo.getString("name"))) {
	...
	public int calculateTotalMemory() throws Exception {
		String yarnRoles = getMethod(InitConfig.CM_ADDR + "/api/v9/clusters/" + getClusterName() + "/services/" + getYarnServiceName() + "/roles");
		Map<String, Object> NodeManagerNumMap = parseNodeManagerNum(yarnRoles);
		String roleConfigGroups = getMethod(InitConfig.CM_ADDR + "/api/v9/clusters/" + getClusterName() 
				   + "/services/" + getYarnServiceName() + "/roleConfigGroups");
		...
		for (int i = 0; i < roleConfigGroups_ja.length(); i++) {
			...
			for (int j = 0; j < config_ja.length(); j++) {
				JSONObject item_jo =config_ja.getJSONObject(j);
				if ("yarn_nodemanager_resource_memory_mb".equals(item_jo.getString("name"))) {
	...
	public String getMethod(String url) throws Exception {
		...
		httpclient.getState().setCredentials( // 服务器认证
				AuthScope.ANY, // 认证域
				new UsernamePasswordCredentials(InitConfig.CM_USER, InitConfig.CM_PSW) // 用户名、密码认证
				);
	...
	public String getYarnServiceName() throws Exception {
		String cluster = getMethod(InitConfig.CM_ADDR + "/api/v9/clusters/" + getClusterName() + "/services");
		JSONObject jsonObject = new JSONObject(cluster);
	...
	public String getRMConfigGroupName() throws Exception {
		String roleConfigGroups = getMethod(InitConfig.CM_ADDR + "/api/v9/clusters/" + getClusterName() + "/services/" + getYarnServiceName() + "/roleConfigGroups");
	...
	public String getRMRoleName() throws Exception {
		String roles = getMethod(InitConfig.CM_ADDR + "/api/v9/clusters/" + getClusterName() + "/services/" + getYarnServiceName() + "/roles");
	 ...
	 * @param yarnroles 通过cms的接口获得的所有yarn角色的信息
	 * @return 返回存角色组名称与数量的map
	 * @throws Exception
	 */
	public Map<String, Object> parseNodeManagerNum(String yarnroles) throws Exception {
		int roleConfigGroupNum;
		Map<String, Object> roleConfigGroupMap = new HashMap<String, Object>();
		JSONArray yarnRoles_ja = (new JSONObject(yarnroles)).getJSONArray("items");
		for (int i = 0; i < yarnRoles_ja.length(); i++) {
			...
			String roleConfigGroupName = yarnRole_jo.getJSONObject("roleConfigGroupRef").getString("roleConfigGroupName");
			if (!roleConfigGroupMap.containsKey(roleConfigGroupName)) {
				roleConfigGroupMap.put(roleConfigGroupName, 1);
			} else {
				roleConfigGroupNum = (int) roleConfigGroupMap.get(roleConfigGroupName) + 1;
	...
	public void putMethod(String url, String entity) throws Exception {
		...
		httpclient1.getState().setCredentials( // 服务器认证
				AuthScope.ANY, // 认证域
				new UsernamePasswordCredentials(InitConfig.CM_USER, InitConfig.CM_PSW) // 用户名、密码认证
				);
	...
	public String refreshQueue_CMS() throws Exception {
		...
		String url = InitConfig.CM_ADDR + "/api/v9/clusters/" + getClusterName() + "/services/" + getYarnServiceName() + "/roleCommands/refresh";	
