...
public class YarnUtil {
	...
	/**
	 * 检测是否存在队列
	 * @param schedulerinfo 通过调用yarn rest api获得的调度器信息，获得是json格式，转为xml再解析
	 * @param queueName 队列名称
	 * @return 返回是否存在调度器
	 * @throws Exception
	 */
	public boolean existQueue(String schedulerinfo, String queueName) throws Exception {
	 ...
	 * @param appid application id
	 * @param appLocalLogPath 日志输出路径
	 * @throws Exception
	 */
	public void getContainersLogs(String appid, String appLocalLogPath) throws Exception {
		...
		remoteRootLogDir = new Path(config.get("yarn.nodemanager.remote-app-log-dir", "/tmp/logs"));
		YarnClient yarnClient = YarnClient.createYarnClient();
	    ...
	    try {
	        ...
	        JavaUtil.log("Logs not available at " + remoteAppLogDir.toString(), appLocalLogPath);
	        JavaUtil.log("Log aggregation has not completed or is not enabled.", appLocalLogPath);
	        return;
	    }
	    ...
	    while (nodeFiles.hasNext()) {
	        ...
	        try {
	            ...
	            while (valueStream != null) {
	                ...
	                JavaUtil.log(StringUtils.repeat("=", containerString.length()), appLocalLogPath);
	                try {
	                    while (true) {
	            	        readAContainerLogsForALogType(valueStream, appLocalLogPath);
	                    }
	...
	public int getNumFinishedApps(String queueName) throws Exception {
	    ...
	    for (int i = 0; i < appslist.size(); i++) {
	    	...
	    	if (queueName.equals(application.getQueue()) && "FINISHED".equals(application.getYarnApplicationState().toString())) {
	    	num++;	
	    	}
	    }
	    yarnClient.close();
	 ...
	 * 检测队列是否改变
	 * @param queueName 队列名称
	 * @param changeName 队列中改变的属性名称
	 * @param changeValue 队列中改变的属性值
	 * @return 返回是否改变
	 * @throws Exception
	 */
	public boolean isQueueChanged(String queueName, String changeName, double changeValue) throws Exception {
		...
		JSONArray queues_ja = (new JSONObject(schedulerInfo)).getJSONObject("scheduler").getJSONObject("schedulerInfo").getJSONObject("queues").getJSONArray("queue");
    ...
    /**
     * kill掉application
     * @param appid 要kill的application id
     * @return
     * @throws Exception
     */
	...
	/**
	 * 解析containerLog的内容
	 * @param valueStream 数据输入流
	 * @param appLogPath 日志输入路径
	 * @throws Exception
	 */
	 public void readAContainerLogsForALogType(DataInputStream valueStream, String appLogPath) throws Exception {
